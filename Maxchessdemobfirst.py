...
#import data from Tkinter program
...
def sget():
    ...
    while True :
        ...
        if text[0:8] == 'bestmove':
            ...
            return mtext
def getboard():
    btxt = ser.readline() #read the move sent by the arduino
    return btxt
...
def bmove(fmove):
    ...
    if brdmove =="a9a9":
        ...
        text = sget()
        print (text)
        """smove holds the text of the move for the computer"""
        smove = text[9:13]
        hint = text[21:25]
...
#  elif valid  make the move and send Fen to board
      ...
      #  print (cmove)
        ...
        # put('stop')
        text = sget()
        #try to stop stuff on screen between moves: print (text)
        smove = text[9:13]
        compfirstmove = text[9:11];
        compsecmove = text[11:13];
        hint = text[21:25]
        if maxchess.addTextMove(smove) != True :
                        stxt = "e"+ str(maxchess.getReason())+move
                        maxchess.printBoard()
                        ...
                        #send the computer moves to the arduino
                        send_string = compfirstmove+" "+compsecmove+" \n"
...
# assume new game
print ("\n Chess Program \n")
#grab the skill level entered from Tkinter
